...
class view:
	...
	def genera_code(self,frame,SSID,Seguridad, Password,Visibilidad):
		...
		if SSID != "" and Password != "":
			...
			mb.showerror("Watch out","Fill in all the fields");
	...
	def ViewGenerateQR(self):
		...
		lblSeg = tk.Label(f2, text = "Security:", font =("verdana", 12));
		lblSeg.place(x = 60, y = 40, anchor ="center");
		cbxSeg = ttk.Combobox(f2,state="readonly",values=["None","WPA","WEP"])
		cbxSeg.current(1)
		cbxSeg.place(x = 120, y = 25);


		lblPwd= tk.Label(f2, text = "Password:", font =("verdana", 12));
		lblPwd.place(x = 65, y = 65, anchor ="center");

		txtPwd = tk.Entry(f2);
		txtPwd.place(x = 120, y = 50, width=145);
		
		lblHide= tk.Label(f2, text = "Hidden NET: ", font =("verdana", 12));
